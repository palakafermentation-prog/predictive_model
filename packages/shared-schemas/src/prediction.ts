...
export const PredictionRequestSchema = z.object({
  ...
  rice_polish_ratio: z.number({ message: "Polish Ratio is required" }).min(30, "Min 30%").max(100, "Max 100%"),
  koji_incubation_hours: z.number({ message: "Incubation Hours is required" }).min(12, "Min 12 hrs").max(60, "Max 60 hrs"),
  moromi_duration_days: z.number({ message: "Duration is required" }).min(10, "Min 10 days").max(120, "Max 120 days"),
